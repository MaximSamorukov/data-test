const Keyboard = {
  ...
  toggleKeyboardLanguage(language) {
    ...
    if (this.properties.shift) {
      ...
      let shiftKey = '';
      elem.forEach((item) => {
        if (item.innerText === 'shift') {
          shiftKey = item;
        };
      });
      const fragment = this._createKeys(language);
      const container = document.querySelector('.keyboard__keys');
      const flag = this.properties.sound === true ? true : false;
      this.properties.sound = this.properties.sound === true ? false : this.properties.sound;
      shiftKey.click();
      container.innerHTML = '';
      ...
      shiftKey.click();
      this.properties.sound = flag === true ? !this.properties.sound : this.properties.sound;
    } else {
  ...
  _toggleVoiceRecognition(context) {
    ...
    if (this.properties.voice) {
      ...
      this.properties.recognition.onresult = function (event) {
        ...
        context.properties.cursor.start = startPosition;
        context.properties.cursor.end = endPosition;
